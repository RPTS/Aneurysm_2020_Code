...
%   x is a n*3 or n*2 matrix where each row contains the coordinates of a vertex.
%   loc is [x, y, z] or [x, y] - the coordinates of the points we're interested in
...
% ---------------------------------------------------------------
    ...
    dist = 0;
    onesMatrix = ones(size(t.Points, 1),1);
    for i = 1:size(loc,1)
        loc2 = loc(i*onesMatrix, :); % similar to repmat but quicker as less checking
        test = (t.Points - loc2);
        test2 = sum(test.*test, 2);
        
        if nargout == 1
            [~,vertex] = min(test2); %this is the fastest version
        elseif nargout == 2
            [dist,vertex] = min(test2);
            dist = sqrt(dist);
        else
